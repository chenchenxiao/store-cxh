                ...
                <c:forEach items="${cart.cartItemsList}" var="cartItems" varStatus="status">
                    ...
                    <div id="product_11345721" data-bind="rowid:1" class="item item_selected ">
                        <div class="item_form clearfix">
                            ...
                            <div class="cell p-price"><span class="price">¥<fmt:formatNumber groupingUsed="false" value="${cartItems.items.price }" maxFractionDigits="2" minFractionDigits="2"/> </span></div>
                            ...
                            <div class="cell p-quantity" for-stock="for-stock-11345721">
                                ...
                                <div class="quantity-form" data-bind="">
                                    <a href="javascript:void(0);" class="decrement" clstag="clickcart|keycount|xincart|diminish1" id="decrement">-</a>
                                        <input type="text" class="quantity-text"  itemPrice="${cartItems.items.price}" itemIds="${cartItems.items.id}" value="${cartItems.itemsNumber }" id="changeQuantity-11345721-1-1-0">
                                    <a href="javascript:void(0);" class="increment" clstag="clickcart|keycount|xincart|add1" id="increment">+</a>
                                </div>
                                 <div class="tips"></div>
                                 <input type="hidden" name="cartId" value="${cartItems.id}"/>
                                 <input type="hidden" name="number" class="number" value="${cartItems.items.number}"/>
                                 <input type="hidden" name="money" class="money" value="${cartItems.money}"/>

                            </div>
                            <div class="cell p-inventory stock-11345721" style="color: red">
                                ￥${cartItems.cost}
                            </div>
                            <div class="cell p-remove"><a id="remove-11345721-1" data-more="removed-87.20-1" clstag="clickcart|keycount|xincart|btndel318558" class="cart-remove" href="${pageContext.request.contextPath}/admin/ordercartItems/deleteByIds?itemIds=${cartItems.items.id}&cartId=${cart.id}">删除</a>
                            </div>
                        </div>
                    </div>
                </c:forEach>
                ...
                <div class="control fdibu fdibucurrent">
                    <span class="delete">
                          ...
                          <a href="javascript:void(0);" clstag="clickcart|keycount|xincart|clearcartlink" id="remove-batch">
                                 <c:if test="${cart.cartItemsList[0] != null}">删除选中的商品</c:if>
                          </a>
                     </span>
                    <span class="shopping">
                  <b>
...
<script type="text/javascript">
    $(".increment").click(function(){//＋
        ...
        if(!r.test(itemsNumber)){
            ...
            $.ajax({
                "url":"${pageContext.request.contextPath}/admin/cartItems/updateItemsNumber",
                "data":{"id":id,"itemsNumber":itemsNumber,"money":money,"cartId":cartId},
                "type":"POST",
